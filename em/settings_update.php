<?php
	...
	$old_uname_n_em_uname_match=0;
	$new_uname_n_old_uname_unmatch=0;
	$old_email_n_em_email_match=0;
	$new_email_n_old_email_unmatch=0;
	$old_pass_n_em_pass_match=0;
	...
	if (!empty($_POST['old_uname']) && !empty($_POST['new_uname']) && !empty($_POST['old_email']) && !empty($_POST['new_email']) && !empty($_POST['old_pass']) && !empty($_POST['new_pass']) && !empty($_POST['confirm_pass']))
	{
		...
		if ($_POST['old_uname']==$em_uname)
		{
			$old_uname_n_em_uname_match=1;
		}
		if ($_POST['new_uname']!=$em_uname)
		{
			$new_uname_n_old_uname_unmatch=1;
		}


		if ($_POST['old_email']==$em_email)
		{
			$old_email_n_em_email_match=1;
		}
		if ($_POST['new_email']!=$em_email)
		{
			$new_email_n_old_email_unmatch=1;
		}


		if (md5($_POST['old_pass'])==$em_pass)
		{
			$old_pass_n_em_pass_match=1;
		}
		if (md5($_POST['new_pass'])!=$em_pass)
		{
		...
		if ($old_uname_n_em_uname_match==1 && $new_uname_n_old_uname_unmatch==1 && $old_email_n_em_email_match==1 && $new_email_n_old_email_unmatch==1 && $old_pass_n_em_pass_match==1 && $new_pass_n_old_pass_unmatch==1 && $new_pass_n_cpass_match==1)
		{
	...
	elseif (!empty($_POST['old_uname']) && !empty($_POST['new_uname']) && !empty($_POST['old_email']) && !empty($_POST['new_email']) && empty($_POST['old_pass']) && empty($_POST['new_pass']) && empty($_POST['confirm_pass']))
	{
		...
		if ($old_uname_n_em_uname_match==1 && $new_uname_n_old_uname_unmatch==1 && $old_email_n_em_email_match==1 && $new_email_n_old_email_unmatch==1)
		{
	...
	elseif (empty($_POST['old_uname']) && empty($_POST['new_uname']) && !empty($_POST['old_email']) && !empty($_POST['new_email']) && !empty($_POST['old_pass']) && !empty($_POST['new_pass']) && !empty($_POST['confirm_pass']))
	{
		...
		if ($old_email_n_em_email_match==1 && $new_email_n_old_email_unmatch==1 && $old_pass_n_em_pass_match==1 && $new_pass_n_old_pass_unmatch==1 && $new_pass_n_cpass_match==1)
		{
			$email_pass_update=1;
		}
	}
	...
	elseif (!empty($_POST['old_uname']) && !empty($_POST['new_uname']) && empty($_POST['old_email']) && empty($_POST['new_email']) && !empty($_POST['old_pass']) && !empty($_POST['new_pass']) && !empty($_POST['confirm_pass']))
	{
		...
		if ($old_uname_n_em_uname_match==1 && $new_uname_n_old_uname_unmatch==1 && $old_pass_n_em_pass_match==1 && $new_pass_n_old_pass_unmatch==1 && $new_pass_n_cpass_match==1)
		{
			$uname_pass_update=1;
		}
	}
	...
	elseif (!empty($_POST['old_uname']) && !empty($_POST['new_uname']) && empty($_POST['old_email']) && empty($_POST['new_email']) && empty($_POST['old_pass']) && empty($_POST['new_pass']) && empty($_POST['confirm_pass']))
	{
		...
		if ($old_uname_n_em_uname_match==1 && $new_uname_n_old_uname_unmatch==1)
		{
			$uname_update=1;
		}
	}
	...
	elseif (empty($_POST['old_uname']) && empty($_POST['new_uname']) && !empty($_POST['old_email']) && !empty($_POST['new_email']) && empty($_POST['old_pass']) && empty($_POST['new_pass']) && empty($_POST['confirm_pass']))
	{
		...
		if ( $old_email_n_em_email_match==1 && $new_email_n_old_email_unmatch==1)
		{
			$email_update=1;
		}
	}
	...
	elseif (empty($_POST['old_uname']) && empty($_POST['new_uname']) && empty($_POST['old_email']) && empty($_POST['new_email']) && !empty($_POST['old_pass']) && !empty($_POST['new_pass']) && !empty($_POST['confirm_pass']))
	{
		...
		if ($old_pass_n_em_pass_match==1 && $new_pass_n_old_pass_unmatch==1 && $new_pass_n_cpass_match==1)
		{
			$pass_update=1;
		}
	}
	...
	if ($uname_email_pass_update==1) 
	{
		...
		$query = "UPDATE employer SET email='$new_email', username='$new_uname', password='$new_pass' WHERE id='$em_id'";
	    ...
	    else
	    {
	    	...
	    	$_SESSION['em_info']['email']=$new_email;
	    	$_SESSION['em_info']['uname']=$new_uname;
	    	$_SESSION['em_info']['pass']=$new_pass;
	...
	elseif ($uname_email_update==1) 
	{
		...
		$query = "UPDATE employer SET email='$new_email', username='$new_uname' WHERE id='$em_id'";
	...
	elseif ($email_pass_update==1) 
	{
		...
		$query = "UPDATE employer SET email='$new_email', password='$new_pass' WHERE id='$em_id'";
	...
	elseif ($uname_pass_update==1) 
	{
		...
		$query = "UPDATE employer SET username='$new_uname', password='$new_pass' WHERE id='$em_id'";
	...
	elseif ($uname_update==1) 
	{
		...
		$query = "UPDATE employer SET username='$new_uname' WHERE id='$em_id'";
	...
	elseif ($email_update==1) 
	{
		...
		$query = "UPDATE employer SET email='$new_email' WHERE id='$em_id'";
	...
	elseif ($pass_update==1) 
	{
		...
		$query = "UPDATE employer SET password='$new_pass' WHERE id='$em_id'";
	...
	if (($uname_email_pass_check==1 || $uname_email_check==1 || $uname_pass_check==1 || $uname_check==1) && $old_uname_n_em_uname_match==0)
	{
		?>
		<script type="text/javascript">
			$.notify({
	...
	elseif(($uname_email_pass_check==1 || $uname_email_check==1 || $email_pass_check==1 || $email_check==1) && $old_email_n_em_email_match==0)
	{
		?>
		<script type="text/javascript">
			$.notify({
	...
	elseif(($uname_email_pass_check==1 || $email_pass_check==1 || $uname_pass_check==1 || $pass_check==1) && $old_pass_n_em_pass_match==0)
	{
		?>
		<script type="text/javascript">
			$.notify({
